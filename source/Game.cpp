    ...
    while (!gameOver())
    {
        ...
        if (disp)
        {
            ...
            for (IDType p(0); p<Constants::Num_Players; ++p)
            {
				...
				Player_Rave *  rave  = dynamic_cast<Player_Rave *>  (_players[p].get());
				...
				if (rave) 
                { 
                    disp->SetParams(p, rave->getParams().getDescription()); 
                    disp->SetResults(p, rave->getResults().getDescription());
                }
            }
