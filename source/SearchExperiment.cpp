...
void SearchExperiment::addPlayer(const std::string & line)
{
    ...
    iss >> player; // the identifier "Player"
    iss >> playerID;
    ...
    else if (playerModelID == PlayerModels::UCT)
    {
        ...
        std::string     playerToMoveMethod;
        std::string     opponentModelScript;
		std::string		playerMoveSelection;
        ...
        int playerToMoveID      = PlayerToMove::getID(playerToMoveMethod);
        int opponentModelID     = PlayerModels::getID(opponentModelScript);
		int playerMoveSelectionID = UCTMoveSelect::getID(playerMoveSelection);
        ...
        params.setSimScripts(playoutScriptID1, playoutScriptID2);
        params.setPlayerToMoveMethod(playerToMoveID);
		params.setRootMoveSelectionMethod(playerMoveSelectionID);
	...
	else if (playerModelID == PlayerModels::Rave)
    {
        int             timeLimitMS;
        double          cValue;
        int             maxTraversals;
		...
		PlayerPtr ravePlayer(new Player_Rave(playerID, params));
        players[playerID].push_back(ravePlayer); 
    }
	else
    {
