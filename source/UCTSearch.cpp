...
void UCTSearch::doSearch(GameState & initialState, std::vector<UnitAction> & move, IDType player)
{
	_maxPlayer = (_params.maxPlayer() == player);
    Timer t;
    t.start();
    ...
    {
		move = _rootNode.bestAvgValueChild(_maxPlayer).getMove();
    }
    ...
    {
		move = _rootNode.bestComboValueChild(_maxPlayer).getMove();
    }
