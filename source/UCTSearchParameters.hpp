...
namespace SparCraft
{
	...
	class UCTMoveSelect: public EnumData<UCTMoveSelect>
    {
	public:
		enum { HighestAvgValue, MostVisited, HighestComboValue, Size};
		static void init()
		{
			setType("RaveMoveSelect");
			names.resize(Size);
			setData(HighestAvgValue,    "HighestAvgValue");
			setData(MostVisited,        "");
			setData(MostVisited,        "MostVisited");
			setData(HighestComboValue,	"HighestComboValue");
		}
	};
}
...
public:
	...
	const std::string & getPlayerTypeName()						const	{ return _playerTypeName; }
	...
	void setPlayerTypeName(const std::string & typeName)				{ _playerTypeName = typeName; }
    ...
    std::vector<std::vector<std::string> > & getDescription()
    {
        if (_desc.size() == 0)
        {
            ...
            ss << _playerTypeName;                                      _desc[1].push_back(ss.str()); ss.str(std::string());
